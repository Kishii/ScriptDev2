...
#define MSG_COMBAT                  "Vous ête en combat !"
#define MSG_LEARNED_SPELL           "Félicitation, vous avez appris vos sorts de classes !"
#define MSG_LEARNED_WEAPON          "Félicitation, vous avez appris vos compétences d'armes !"
#define MSG_LEARNED_LANGUAGES       "Félicitation, vous avez appris toute les langues !"
#define MSG_MAXED_WEAPON            "Félicitation, vous avez vos compétences d'armes !"
#define MSG_MAXED_LANGUAGES         "Félicitation, vous avez toute les langues aux max !"
#define MSG_MAXED_RIDING            "Félicitation, vous avez votre compétence de monte aux max !"
...
bool GossipHello_custom_npc_spellmaster(Player* pPlayer, Creature* pCreature)
{
    ...
    bool LearnAllWeapons            = SCConfig.GetBoolDefault("Spellmaster.LearnAllWeapons", true);
    bool LearnAllLanguages          = SCConfig.GetBoolDefault("Spellmaster.LearnAllLanguages", true);
    bool MaxOutWeaponSkills         = SCConfig.GetBoolDefault("Spellmaster.MaxOutWeaponSkills", true);
    bool MaxOutRidingSkills         = SCConfig.GetBoolDefault("Spellmaster.MaxOutRidingSkills", true);
    bool MaxOutLanguageSkills       = SCConfig.GetBoolDefault("Spellmaster.MaxOutAllLanguageSkills", true);


    // Guerrier
    if(pPlayer->getClass() == CLASS_WARRIOR)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Guerrier ?"            , GOSSIP_SENDER_MAIN, 1000);
    // Paladin
    if(pPlayer->getClass() == CLASS_PALADIN)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Paladin ?"            , GOSSIP_SENDER_MAIN, 2000);
    // Chasseur
    if(pPlayer->getClass() == CLASS_HUNTER)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Chasseur ?"            , GOSSIP_SENDER_MAIN, 3000);
    // Voleur
    if(pPlayer->getClass() == CLASS_ROGUE)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Voleur ?"            , GOSSIP_SENDER_MAIN, 4000);
    // Prêtre
    if(pPlayer->getClass() == CLASS_PRIEST)
	    pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Pretre ?"            , GOSSIP_SENDER_MAIN, 5000);
    // Chevalier de la mort
    if(pPlayer->getClass() == CLASS_DEATHKNIGHT)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Chevalier de la Mort ?"    , GOSSIP_SENDER_MAIN, 6000);
    // Chaman
    if(pPlayer->getClass() == CLASS_SHAMAN)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Shaman ?"            , GOSSIP_SENDER_MAIN, 7000);
    // Mage
    if(pPlayer->getClass() == CLASS_MAGE)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Mage ?"            , GOSSIP_SENDER_MAIN, 8000);
    // Démoniste
    if(pPlayer->getClass() == CLASS_WARLOCK)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Demoniste ?"            , GOSSIP_SENDER_MAIN, 9000);
    // Druide
    if(pPlayer->getClass() == CLASS_DRUID)
        pPlayer->ADD_GOSSIP_ITEM( 3, "Apprendre tout les sorts Druide ?"            , GOSSIP_SENDER_MAIN, 10000);
    // Compétence d'armes
    if(LearnAllWeapons)
        pPlayer->ADD_GOSSIP_ITEM( 9, "Apprendre les compétences en arme de ma classe ?"        , GOSSIP_SENDER_MAIN, 11000);
    // Compétence d'armes aux max
    if(MaxOutWeaponSkills)
        pPlayer->ADD_GOSSIP_ITEM( 9, "Avoir les competences en arme au maximum ?"            , GOSSIP_SENDER_MAIN, 12000);
    // Compétence de monte aux max
    if(MaxOutRidingSkills)
        pPlayer->ADD_GOSSIP_ITEM( 2, "Avoir la competence monture au maximum ?"            , GOSSIP_SENDER_MAIN, 13000);
    // Apprendre toutes les langues
    if(LearnAllLanguages)
        pPlayer->ADD_GOSSIP_ITEM( 0, "Apprendre toutes les langues ?"                , GOSSIP_SENDER_MAIN, 14000);
    // Langues au max
    if(MaxOutLanguageSkills)
        pPlayer->ADD_GOSSIP_ITEM( 7, "Avoir les language au maximum ?"            , GOSSIP_SENDER_MAIN, 15000);
		pPlayer->ADD_GOSSIP_ITEM( 0, "Non, merci."                    , GOSSIP_SENDER_MAIN, 99000);
...
void SendDefaultMenu_custom_npc_spellmaster(Player* pPlayer, Creature* pCreature, uint32 uiAction)
{
	...
	switch(uiAction)
	{
		case 1000: // Guerrier
			...
			{
				pPlayer->learnSpell(8198, false);  // Coup de tonnerre (Rang 2)
				pPlayer->learnSpell(676, false);   // Désarmement
				pPlayer->learnSpell(694, false);   // Coup railleur
				pPlayer->learnSpell(285, false);   // Frappe héroïque (Rang 3)
				pPlayer->learnSpell(2565, false);  // Maîtrise du blocage
				pPlayer->learnSpell(1160, false);  // Cri démoralisant (Rang 1)
				pPlayer->learnSpell(6572, false);  // Vengeance (Rang 1)
				pPlayer->learnSpell(72, false);    // Coup de bouclier
				pPlayer->learnSpell(5242, false);  // Cri de guerre (Rang 2)
				pPlayer->learnSpell(7384, false);  // Fulgurance
				pPlayer->learnSpell(7386, false);  // Fracasser armure
				pPlayer->learnSpell(71, false);    // Posture défensive
				pPlayer->learnSpell(6546, false);  // Pourfendre (Rang 2)
				pPlayer->learnSpell(355, false);   // Provocation
				pPlayer->learnSpell(2687, false);  // Rage sanguinaire
				pPlayer->learnSpell(1715, false);  // Brise-genou
				pPlayer->learnSpell(34428, false); // Ivresse de la victoire
				pPlayer->learnSpell(100, false);   // Charge (Rang 1)
				pPlayer->learnSpell(2457, false);  // Posture de combat
			
				pPlayer->CLOSE_GOSSIP_MENU();
				pCreature->MonsterWhisper(MSG_LEARNED_SPELL, pPlayer->GetGUID());
			}
			else if (pPlayer->getLevel() == 80)
			{
			...
			{
				pCreature->MonsterWhisper("Vous n'avez pas le level requis ( 19 / 60 / 80 ) !", pPlayer->GetGUID());
				pPlayer->CLOSE_GOSSIP_MENU();
			}
		break;
		...
		case 2000: // Paladin
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(1044, false);  // Main de liberté
				pPlayer->learnSpell(7294, false);  // Aura de vindicte (Rang 1)
				pPlayer->learnSpell(25780, false); // Fureur vertueuse
				pPlayer->learnSpell(62124, false); // Main de rétribution
				pPlayer->learnSpell(19742, false); // Bénédiction de sagesse (Rang 1)
				pPlayer->learnSpell(31789, false); // Défense vertueuse
				pPlayer->learnSpell(647, false);   // Lumière sacrée (Rang 3)
				pPlayer->learnSpell(19834, false); // Bénédiction de puissance (Rang 2)
				pPlayer->learnSpell(53408, false); // Jugement de sagesse
				pPlayer->learnSpell(7328, false);  // Rédemption (Rang 1)
				pPlayer->learnSpell(10290, false); // Aura de dévotion (Rang 2)
				pPlayer->learnSpell(633, false);   // Imposition des mains (Rang 1)
				pPlayer->learnSpell(1022, false);  // Main de protection (Rang 1)
				pPlayer->learnSpell(853, false);   // Marteau de la justice (Rang 1)
				pPlayer->learnSpell(1152, false);  // Purification
				pPlayer->learnSpell(498, false);   // Protection divine
				pPlayer->learnSpell(20271, false); // Jugement de lumière
				pPlayer->learnSpell(21084, false); // Sceau de piété
			...
			else if (pPlayer->getLevel() == 80)
			{
				...
				if(pPlayer->getRace() == 10) 
				pPlayer->learnSpell(53736, false);      // Seal of Corruption
				else if(pPlayer->getRace() == 1 || 3 || 11) 
				pPlayer->learnSpell(31801, false);      // Seal of Vengeance
				if(pPlayer->getRace() == 10) 
		...
		case 3000: // Chasseur
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(14318, false); // Aspect du faucon (Rang 2)
				pPlayer->learnSpell(2643, false);  // Flèches multiples (Rang 1)
				pPlayer->learnSpell(13550, false); // Mosure de serpent (Rang 3)
				pPlayer->learnSpell(19884, false); // Pistage des morts-vivants
				pPlayer->learnSpell(5118, false);  // Aspect du guépard
				pPlayer->learnSpell(14261, false); // Attaque du raptor (Rang 3)
				pPlayer->learnSpell(1495, false);  // Morsure de la mangouste (Rang 1)
				pPlayer->learnSpell(13795, false); // Piège d'immolation (Rang 1)
				pPlayer->learnSpell(1513, false);  // Effrayer une bête (Rang 1)
				pPlayer->learnSpell(6197, false);  // Oeil d'aigle
				pPlayer->learnSpell(1002, false);  // Yeux de la bête
				pPlayer->learnSpell(2974, false);  // Coupure d'ailes
				pPlayer->learnSpell(136, false);   // Guérison du familier (Rang 1)
				pPlayer->learnSpell(14281, false); // Tir des arcanes (Rang 2)
				pPlayer->learnSpell(20736, false); // Trait provocateur (Rang 1)
				pPlayer->learnSpell(883, false);   // Appel du familier
				pPlayer->learnSpell(1515, false);  // Apprivoise une bête 
				pPlayer->learnSpell(6991, false);  // Nourrir le familier
				pPlayer->learnSpell(19883, false); // Pistage des humanoïdes
				pPlayer->learnSpell(2641, false);  // Renvoyer le familier
				pPlayer->learnSpell(982, false);   // Ressusciter le familier
				pPlayer->learnSpell(5116, false);  // Trait de choc
				pPlayer->learnSpell(1130, false);  // Marque du chasseur (Rang 1)
				pPlayer->learnSpell(13163, false); // Aspect du singe
				pPlayer->learnSpell(1494, false);  // Pistage des bêtes
				pPlayer->learnSpell(75, false);    //Tir automatique
		...
		case 4000: // Voleur
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(8676, false); // Embuscade (Rang 1)
				pPlayer->learnSpell(1804, false); // Crochetage
				pPlayer->learnSpell(6761, false); // Eviscération( Rang 3)
				pPlayer->learnSpell(1966, false); // Feinte (Rang 1)
				pPlayer->learnSpell(1758, false); // Attaque pernicieuse (Rang 3)
				pPlayer->learnSpell(8647, false); // Exposer l'armure
				pPlayer->learnSpell(703, false);  // Garrot (Rang 1)
				pPlayer->learnSpell(2589, false); // Attaque sournoise (Rang 2)
				pPlayer->learnSpell(1766, false); // Coup de pied
				pPlayer->learnSpell(6770, false); // Assomer (Rang 1)
				pPlayer->learnSpell(5171, false); // Débiter (Rang 1)
				pPlayer->learnSpell(2983, false); // Sprint (Rang 1)
				pPlayer->learnSpell(5277, false); // Evasion (Rang 1)
				pPlayer->learnSpell(1776, false); // Suriner
				pPlayer->learnSpell(921, false);  // Vol à la tire
				pPlayer->learnSpell(1784, false); // Camouflage
		...
		case 5000: // Prêtre
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(527, false);   // Dissipation de la magie (Rang 1)
				pPlayer->learnSpell(970, false);   // Mot de l'ombre : douleur (Rang 3)
				pPlayer->learnSpell(600, false);   // Mot de pouvoir : bouclier (Rang 3)
				pPlayer->learnSpell(8102, false);  // Attaque mentale (Rang 2)
				pPlayer->learnSpell(2054, false);  // Soins (Rang 1)
				pPlayer->learnSpell(598, false);   // Châtiment (Rang 3)
				pPlayer->learnSpell(8122, false);  // Cri psychique (Rang 1)
				pPlayer->learnSpell(528, false);   // Guérison des maladies
				pPlayer->learnSpell(6074, false);  // Rénovation (Rang 2)
				pPlayer->learnSpell(588, false);   // Feu intérieur (Rang 1)
				pPlayer->learnSpell(1244, false);  // Mot de pouvoir : robustesse (Rang 2)
				pPlayer->learnSpell(2006, false);  // Résurrection (Rang 1)
				pPlayer->learnSpell(2053, false);  // Soins inférieurs (Rang 3)
				pPlayer->learnSpell(586, false);   // Oubli
		...
		case 6000: // Chevalier de la mort
			if (pPlayer->getLevel() == 80)
			{
				...
				pPlayer->learnSpell(49895, false);      // Death Coil [Rank 5]
			...
			{
				pCreature->MonsterWhisper("Vous n'avez pas le level requis ( 60 / 80 ) !", pPlayer->GetGUID());
				pPlayer->CLOSE_GOSSIP_MENU();
			}
		break;
		...
		case 7000: // Chaman
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(6390, false);  // Totem de griffes de pierre (Rang 2)
				pPlayer->learnSpell(8143, false);  // Totem de Séisme
				pPlayer->learnSpell(913, false);   // Vague de soins (Rang 4)
				pPlayer->learnSpell(8019, false);  // Arme Croque-roc (Rang 3
				pPlayer->learnSpell(325, false);   // Bouclier de Foudre (Rang 2)
				pPlayer->learnSpell(57994, false); // Cisaille de vent
				pPlayer->learnSpell(526, false);   // Guérison des toxines
				pPlayer->learnSpell(2645, false);  // Loup fantôme
				pPlayer->learnSpell(548, false);   // Eclair (Rang 3)
				pPlayer->learnSpell(8045, false);  // Horion de terre (Rang 3)
				pPlayer->learnSpell(8154, false);  // Totem de peau de pierre (Rang 2)
				pPlayer->learnSpell(2008, false);  // Esprit ancestral (Rang 1)
				pPlayer->learnSpell(370, false);   // Expiation (Rang 1)
				pPlayer->learnSpell(1535, false);  // Nova de feu (Rang 1)
				pPlayer->learnSpell(8075, false);  // Totem de force de la terre (Rang 1)
				pPlayer->learnSpell(3599, false);  // Totem incendiaire (Rang 1)
				pPlayer->learnSpell(8027, false);  // Arme Langue de feu (Rang 2)
				pPlayer->learnSpell(8052, false);  // Horion de flammes (Rang 2)
		...
		case 8000: // Mage
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(1008, false); // Amplification de la magie (Rang 1)
				pPlayer->learnSpell(3140, false); // Boule de feu (Rang 1)
				pPlayer->learnSpell(475, false);  // Délivrance de la malédiction
				pPlayer->learnSpell(2120, false); // Choc de flammes (Rang 1)
				pPlayer->learnSpell(5144, false); // Projectiles des arcanes (Rang 2)
				pPlayer->learnSpell(837, false);  // Eclair de givre (Rang 3)
				pPlayer->learnSpell(1449, false); // Explosion des arcanes (Rang 1)
				pPlayer->learnSpell(1460, false); // Intelligence des arcanes (Rang 2)
				pPlayer->learnSpell(2137, false); // Trait de feu (Rang 2)
				pPlayer->learnSpell(604, false);  // Atténuation de la magie (Rang 1)
				pPlayer->learnSpell(130, false);  // Chute lente
				pPlayer->learnSpell(597, false);  // Invocation de nourriture (Rang 2)
				pPlayer->learnSpell(5505, false); // Invocation d'eau (Rang 2)
				pPlayer->learnSpell(7300, false); // Armure de givre (Rang 2)
				pPlayer->learnSpell(122, false);  // Nova de givre (Rang 1)
				pPlayer->learnSpell(118, false);  // Métamorphose (Rang 1)
		...
		case 9000: // Démoniste
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(5676, false); // Douleur brûlante (Rang 1)
				pPlayer->learnSpell(1014, false); // Malédiction d'agonie (Rang 2)
				pPlayer->learnSpell(693, false);  // Création de pierre d'âme (Rang 1)
				pPlayer->learnSpell(1455, false); // Connexion (Rang 2)
				pPlayer->learnSpell(5697, false); // Respiration interminable
				pPlayer->learnSpell(6222, false); // Corruption (Rang 2)
				pPlayer->learnSpell(689, false);  // Drain de vie (Rang 1)
				pPlayer->learnSpell(755, false);  // Captation de vie (Rang 1)
				pPlayer->learnSpell(1108, false); // Malédiction de faiblesse (Rang 2)
				pPlayer->learnSpell(705, false);  // Trait de l'ombre (Rang 3)
				pPlayer->learnSpell(6201, false); // Création de pierre de soins (Rang 1)
				pPlayer->learnSpell(1120, false); // Drain d'âme (Rang 1) 
				pPlayer->learnSpell(707, false);  // Immolation (Rang 2)
				pPlayer->learnSpell(697, false);  // Invocation : Marcheur du vide
				pPlayer->learnSpell(696, false);  // Peau de démon (Rang 2)
				pPlayer->learnSpell(5782, false); // Peur (Rang 1)
				pPlayer->learnSpell(688, false);  // Invocation : Diablotin
		...
		case 10000: // Druid Spells
			if (pPlayer->getLevel() == 19)
			{
				pPlayer->learnSpell(16810, false); // Emprise de la nature (Rang 2)
				pPlayer->learnSpell(2637, false);  // Hibernation (Rang 1)
				pPlayer->learnSpell(770, false);   // Lucioles
				pPlayer->learnSpell(16857, false); // Lucioles (farouche)
				pPlayer->learnSpell(6808, false);  // Mutiler (Rang 2)
				pPlayer->learnSpell(8938, false);  // Rétablissement (Rang 2)
				pPlayer->learnSpell(1062, false);  // Sarments (Rang 2)
				pPlayer->learnSpell(779, false);   // Balayage (ours) (Rang 1)
				pPlayer->learnSpell(8925, false);  // Eclat lunaire (Rang 3)
				pPlayer->learnSpell(1066, false);  // Forme aquatique (changeforme)
				pPlayer->learnSpell(783, false);   // Forme de voyage (changeforme)
				pPlayer->learnSpell(1430, false);  // Récupération (Rang 3)
				pPlayer->learnSpell(5178, false);  // Colère (Rang 3)
				pPlayer->learnSpell(782, false);   // Epines (Rang 2)
				pPlayer->learnSpell(8946, false);  // Guérison du poison
				pPlayer->learnSpell(5211, false);  // Sonner (Rang 1)
				pPlayer->learnSpell(5187, false);  // Toucher guérisseur (Rang 3)
				pPlayer->learnSpell(5229, false);  // Enrager
				pPlayer->learnSpell(50769, false); // Ressusciter (Rang 1)
				pPlayer->learnSpell(5487, false);  // Forme d'ours (changeforme)
				pPlayer->learnSpell(6795, false);  // Grondement
				pPlayer->learnSpell(5232, false);  // Marque du fauve (Rang 2)
				pPlayer->learnSpell(99, false);    // Rugissement démoralisant (Rang 1)
				pPlayer->learnSpell(18960, false); // Téléportation : Reflet-de-lune
		...
		case 11000: // Compétences d'armes
		...
		case 13000: // Compétence de monte aux max
		...
		case 14000: // Apprendre toute les languages
