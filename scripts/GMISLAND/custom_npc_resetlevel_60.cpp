...
bool GossipHello_custom_npc_resetlevel_60(Player* pPlayer, Creature* pCreature)
{
Config SCConfig;
if(!SCConfig.SetSource(_SCRIPTDEV2_CONFIG))
error_log("ScriptDev2 : Unable to open configuration file");

    pPlayer->ADD_GOSSIP_ITEM(0, "Voulez vous être level 60 ?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool GossipSelect_custom_npc_resetlevel_60(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    Config SCConfig;
if(!SCConfig.SetSource(_SCRIPTDEV2_CONFIG))
error_log("ScriptDev2 : Unable to open configuration file");
...
pCreature->MonsterWhisper("Vous avez ête mis level 60 !", pPlayer->GetGUID());
...
void AddSC_custom_npc_resetlevel_60()
{
    Script *newscript;

    newscript = new Script;
    newscript->Name = "custom_npc_resetlevel_60";
    newscript->pGossipHello = &GossipHello_custom_npc_resetlevel_60;
    newscript->pGossipSelect = &GossipSelect_custom_npc_resetlevel_60;
